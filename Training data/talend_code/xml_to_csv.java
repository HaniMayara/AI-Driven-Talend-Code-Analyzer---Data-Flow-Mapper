...
package migrationubci.xml_to_csv_0_1;
...
public class xml_to_csv implements TalendJob {
	...
	public static class csvStruct implements routines.system.IPersistableRow<csvStruct> {
		final static byte[] commonByteArrayLock_MIGRATIONUBCI_xml_to_csv = new byte[0];
		static byte[] commonByteArray_MIGRATIONUBCI_xml_to_csv = new byte[0];
		...
		private String readString(ObjectInputStream dis) throws IOException {
			...
			if (length == -1) {
				...
				if (length > commonByteArray_MIGRATIONUBCI_xml_to_csv.length) {
					if (length < 1024 && commonByteArray_MIGRATIONUBCI_xml_to_csv.length == 0) {
						commonByteArray_MIGRATIONUBCI_xml_to_csv = new byte[1024];
					} else {
						commonByteArray_MIGRATIONUBCI_xml_to_csv = new byte[2 * length];
					}
				}
				dis.readFully(commonByteArray_MIGRATIONUBCI_xml_to_csv, 0, length);
				strReturn = new String(commonByteArray_MIGRATIONUBCI_xml_to_csv, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		private String readString(org.jboss.marshalling.Unmarshaller unmarshaller) throws IOException {
			...
			if (length == -1) {
				...
				unmarshaller.readFully(commonByteArray_MIGRATIONUBCI_xml_to_csv, 0, length);
				strReturn = new String(commonByteArray_MIGRATIONUBCI_xml_to_csv, 0, length, utf8Charset);
			}
			return strReturn;
		}
		...
		public void readData(ObjectInputStream dis) {
			...
			synchronized (commonByteArrayLock_MIGRATIONUBCI_xml_to_csv) {
		...
		try {
			...
			if (resumeIt || globalResumeTicket) { // start the resume
				...
				fileName_tFileOutputDelimited_1 = (new java.io.File(
						"C:/Program Files (x86)/TOS_DI-8.0.1/studio/workspace/test_xml_csv1.csv")).getAbsolutePath()
								.replace("\\", "/");
...
// # Outputs initialization
				csvStruct csv_tmp = new csvStruct();
// ###############################
					...
					for (org.dom4j.Node temp_tFileInputXML_1 : nodeList_tFileInputXML_1) {
						...
						try {
							...
							if (xml_api_tFileInputXML_1.isDefNull(node_tFileInputXML_1)) {
								...
								row2.salary = str_tFileInputXML_1;
							}
...
// # Output table : 'csv'
								csv_tmp.dept = row2.dept;
								csv_tmp.name = row2.name;
								csv_tmp.salary = row2.salary;
								csv = csv_tmp;
// ###############################
...
// Start of branch "csv"
							if (csv != null) {
								...
								if (csv.dept != null) {
									sb_tFileOutputDelimited_1.append(csv.dept);
								}
								sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
								if (csv.name != null) {
									sb_tFileOutputDelimited_1.append(csv.name);
								}
								sb_tFileOutputDelimited_1.append(OUT_DELIM_tFileOutputDelimited_1);
								if (csv.salary != null) {
									sb_tFileOutputDelimited_1.append(csv.salary);
								}
	...
	public int runJobInTOS(String[] args) {
		...
		try {
			...
			java.io.InputStream inContext = xml_to_csv.class.getClassLoader()
					.getResourceAsStream("migrationubci/xml_to_csv_0_1/contexts/" + contextStr + ".properties");
			if (inContext == null) {
...
/************************************************************************************************
 * 62209 characters generated by Talend Open Studio for Data Integration on the
 * 13 février 2025 à 16:36:53 CET
 ************************************************************************************************/